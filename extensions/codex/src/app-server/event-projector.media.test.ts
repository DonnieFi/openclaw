...
describe("CodexAppServerEventProjector media projection", () => {
  ...
  it.each([
    { type: "custom_tool_call_output", mimeType: "image/png" },
    { type: "function_call_output", mimeType: "application/octet-stream" },
  ])(
    "saves nested images from raw $type ($mimeType) without generation side effects",
    async ({ type, mimeType }) => {
      const projector = await createProjector();
