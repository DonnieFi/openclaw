...
import { AGENT_TEMPLATE_ROLES, agentTemplateManifestSchema } from "../agent-template-schema.js";
...
const SystemAgentToolSchema = Type.Object({
  ...
  role: Type.Optional(
    stringEnum([...AGENT_TEMPLATE_ROLES], {
...
function operationForAction(params: Record<string, unknown>): SystemAgentOperation {
  ...
  switch (action) {
    ...
    case "create_agent": {
      const role = agentTemplateManifestSchema.shape.role.optional().safeParse(params.role);
      if (!role.success) {
        throw new ToolInputError(
          `openclaw: unknown role; choose ${AGENT_TEMPLATE_ROLES.join(", ")}`,
        );
      }
      const workspace = readToolStringParam(params, "workspace")?.trim();
      const model = readToolStringParam(params, "model")?.trim();
