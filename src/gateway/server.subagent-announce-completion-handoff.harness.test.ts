...
describe("public completion handoff real Gateway admission", () => {
  ...
  it(
    "keeps registry custody pending when retained handoff replay fails",
    { timeout: 30_000 },
    async () => {
      const held = createDeferred();
      agentCommandMock.mockImplementationOnce(async (input: unknown) => {
      ...
      registerSubagentRun({
        ...
        requesterSessionKey,
        requesterDisplayKey: requesterSessionKey,
        task: "handoff failure proof",
        cleanup: "keep",
        expectsCompletionMessage: true,
        spawnMode: "run",
      });
      await settleSubagentRegistryPersistenceWork();
      expect(getSubagentRunByRunId(childRunId)?.delivery?.status ?? "pending").toBe("pending");

      const original = dispatchGatewayMethodInProcess<Record<string, unknown>>(
        "agent",
        agentParams("failed completion handoff"),
        {
          expectFinal: true,
          forceSyntheticClient: true,
          operatorRoleActor: { kind: "system" },
      ...
      // Let the failed Gateway run release root work before the retry announce.
      await settleSubagentRegistryPersistenceWork();
      ...
      const live = getSubagentRunByRunId(childRunId);
      const stored = loadSubagentRegistryFromSqlite().get(childRunId);
      expect(live?.delivery?.status ?? "pending").not.toBe("delivered");
      expect(stored?.delivery?.status ?? "pending").not.toBe("delivered");
      expect(live?.runId).toBe(childRunId);
      expect(stored?.runId).toBe(childRunId);
